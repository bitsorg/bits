...
| `--build-nice-boost-after SECONDS` | With `--build-nice` on **native** builds, a watchdog renices a build that has been running longer than this and was niced down by the ladder back up to top priority — one straggler at a time — so a single long low-priority compile (e.g. a heavy Fortran/C++ translation unit) does not drag out the end of the build. Raising priority requires privilege (root / `CAP_SYS_NICE`); it is a logged no-op otherwise. Docker builds are governed by cgroup `--cpu-shares` instead and are not affected. `0` disables. Default: 600. |
