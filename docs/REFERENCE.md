...
| `--build-nice-boost-after SECONDS` | With `--build-nice`, a watchdog boosts a build that has been running longer than this and was backed off by the ladder back up to top priority — one straggler at a time — so a single long low-priority compile (e.g. a heavy Fortran/C++ translation unit) does not drag out the end of the build. Native builds are reniced toward 0 (requires privilege — root / `CAP_SYS_NICE` — and is a logged no-op otherwise); `--docker`/podman builds run in a named container (`bits-build-<pkg>-<id>`) whose cgroup CPU weight is restored with `docker update --cpu-shares=1024 <name>`. `0` disables. Default: 600. |
