...
def deps_graph(specs, package, runtime_only=False):
  """Return a canonical {pkg: [direct deps]} for only this package's closure.

  Visit each node once, then sort node names and deduplicated edges for stable
  JSON regardless of hash seed, recipe ordering or unrelated build targets.
  No topological sort is needed. Runtime graphs include untracked runtime deps.
  Virtual defaults-release is excluded from both metadata and Makefile output.
  """
  graph, todo = {}, [package]
  while todo:
    p = todo.pop()
    if p in graph or p == "defaults-release":
      continue
    spec = specs[p]
    if runtime_only:
      deps = set(spec.get("runtime_requires", ())) | set(spec.get("untracked_requires", ()))
    else:
      deps = set(spec["requires"])
    # Names without a spec (e.g. an untracked dep satisfied by the system and
    # disabled during resolution) are not part of the closure.
    deps = {d for d in deps if d in specs and d != "defaults-release"}
    graph[p] = sorted(deps)
    todo.extend(dep for dep in deps if dep not in graph)
  return {p: graph[p] for p in sorted(graph)}


def _makefile_order(graph):
  """Dependency-first order with alphabetical ties, local to Makefile export."""
  users = {p: [] for p in graph}
  remaining = {p: len(deps) for p, deps in graph.items()}
  for p, deps in graph.items():
    for dep in deps:
      users[dep].append(p)
  ready = [p for p in graph if not remaining[p]]
  heapify(ready)
  order = []
  while ready:
    p = heappop(ready)
    ...
    for user in users[p]:
      ...
      if not remaining[user]:
        heappush(ready, user)
  dieOnError(len(order) != len(graph), "Dependency cycle detected in Makefile export")
  return order


def deps_makefile(specs, package, runtime_only=False, header=True):
  """Return ``deps_graph`` as Makefile rules (``pkg: dep1 dep2``), no recipes.

  *header* adds the "generated by" comment line, which suits a file written to
  disk but not a value embedded in .meta.json.
  """
  lines = ["# Dependencies of %s%s, generated by `bits deps --outmake`."
           % (package, " (runtime only)" if runtime_only else "")] if header else []
  graph = deps_graph(specs, package, runtime_only)
  order = _makefile_order(graph)
  position = {p: i for i, p in enumerate(order)}
  for p in order:
    deps = sorted(graph[p], key=position.get)
