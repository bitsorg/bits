...
def doBuild(args, parser):
  ...
  with DockerRunner(args.dockerImage, args.docker_extra_args, extra_env={"BITS_CONFIG_DIR": "/alidist" if args.docker else os.path.abspath(args.configDir)}, extra_volumes=[f"{os.path.abspath(args.configDir)}:/alidist:ro"] if args.docker else []) as getstatusoutput_docker:
