...
class Tar(SCM):
  ...
  def checkedOutCommitName(self, directory):
    return tar(("rev-parse", "HEAD"), directory)

  def branchOrRef(self, directory):
    out = tar(("rev-parse", "--abbrev-ref", "HEAD"), directory=directory)
    if out == "HEAD":
      out = tat(("rev-parse", "HEAD"), directory)[:10]
    return out
  ...
  def cloneReferenceCmd(self, spec, referenceRepo, usePartialClone):
    url = self.archive + spec[name] + "-" + spec[version] + spec["type"]
