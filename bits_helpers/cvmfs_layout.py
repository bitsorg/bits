...
``install_path`` / ``module_path`` are the resolved absolutes
...
def resolve_cvmfs_layout(defaults_meta, architecture):
    ...
    Keys: cvmfs_dir, install_dir, module_dir, install_path, module_path.
    Returns None when none of cvmfs_dir / install_dir / module_dir is set, so
    builds that don't opt in are completely unaffected.
    """
    ...
    if not (cvmfs_dir or install_dir or module_dir):
        return None

    subst = {"architecture": architecture}
    cvmfs_dir = _render(cvmfs_dir or "", subst)
    # Sensible defaults keep the install/module dirs arch-scoped if only
    # cvmfs_dir was given.
    install_dir = _render(install_dir or "%(architecture)s", subst)
    module_dir = _render(module_dir or "%(architecture)s/modules", subst)

    install_path = os.path.join(cvmfs_dir, install_dir) if cvmfs_dir else install_dir
    module_path = os.path.join(cvmfs_dir, module_dir) if cvmfs_dir else module_dir
    return {
        "cvmfs_dir": cvmfs_dir,
